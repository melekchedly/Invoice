...
const ids=productIds.data
  ...
  console.log(order)
    try {
      ...
      if (clientFullName) {
          const clients = await Client.find({ fullName: { $regex: clientFullName, $options: 'i' } });
          clientIds = clients.map(client => client._id);
      }

      if (companyName) {
          const sellers = await Seller.find({ companyName: { $regex: companyName, $options: 'i' } });
...
const createInvoices= async(ArrayInvoices)=>{
    ...
    const { Fullname, address, phone, companyname, addressCom=address } = invoice;
    ...
    const savedClient= await Client.create({fullName:Fullname,address,phone})
    const savedClientId =savedClient._id
    const savedSeller= await Seller.create({companyName:companyname,address:addressCom})
   
    const savedSellerId =savedSeller._id
     const theInvoice= await Invoice.create({client:savedClientId,seller:savedSellerId,products:savedProductIds,totalPrice})
